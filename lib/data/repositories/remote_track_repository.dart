...
class RemoteTrackRepository implements TrackRepository {
  ...
  final Map<String, int> _latestFetchByKey = {};
  ...
  Future<Result<List<Track>>> getCatalog({
    ...
    _latestFetchByKey[key] = generation;
    ...
    try {
      ...
      for (final track in raw) {
        final resourceType = (track.metadata['resourceType'] as String?)
            ?.toLowerCase();
      ...
      if (_latestFetchByKey[key] == generation) {
        _cache[key] = merged;
      }
      return Ok(merged);
      ...
      return Err(
        code != null
            ? ServerFailure(code)
            : NetworkFailure(e.cause ?? 'network'),
      );
    } catch (e) {
